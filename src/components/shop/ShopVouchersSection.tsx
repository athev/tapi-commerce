...
const ShopVouchersSection = ({
  ...
  useEffect(() => {
    const fetchVouchers = async () => {
      const {
        data,
        error
      } = await supabase.from('vouchers').select('*').eq('created_by', sellerId).eq('is_active', true).limit(10);
      if (!error && data) {
        setVouchers(data);
      }
      setIsLoading(false);
  ...
  return <div className="bg-card">
      <div className="px-4 py-[5px]">
        {/* Header */}
        <div className="flex items-center justify-between mb-2 my-0 py-0">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-4 px-4 pb-1">
          {vouchers.map(voucher => <div key={voucher.id} className="flex-shrink-0 w-[160px] bg-gradient-to-r from-primary/5 to-transparent border border-primary/20 rounded-lg overflow-hidden flex">
              {/* Left - Discount */}
              ...
              <div className="flex-1 p-1.5 flex flex-col justify-between min-w-0">
                <div className="text-[9px] text-muted-foreground line-clamp-2 leading-tight">
                  {voucher.min_purchase_amount ? `Đơn tối thiểu ${(voucher.min_purchase_amount / 1000).toFixed(0)}k` : "Không giới hạn"}
                </div>
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <Button variant="ghost" size="sm" className="h-4 px-1.5 text-[9px] text-primary hover:text-primary hover:bg-primary/10" onClick={() => handleSaveVoucher(voucher.code)}>
                    Lưu
                  </Button>
                </div>
              </div>
