...
interface EnhancedPriceSectionProps {
  product: any;
...
const EnhancedPriceSection = ({
  ...
  const getProductTypeInfo = (type: string) => {
    const typeInfo = {
      file_download: { icon: Download, label: 'File tải về' },
      shared_account: { icon: Users, label: 'Tài khoản dùng chung' },
      upgrade_account_no_pass: { icon: User, label: 'Nâng cấp tài khoản' },
      upgrade_account_with_pass: { icon: FileText, label: 'Nâng cấp tài khoản' },
      license_key_delivery: { icon: Key, label: 'Mã kích hoạt' }
    };
  ...
  return (
    <div className="space-y-4">
      {/* Price - NO CARD, just border-y */}
      <div className="py-4 border-y space-y-2">
        <div className="flex items-baseline gap-3 flex-wrap">
          <span className="text-4xl md:text-5xl font-black text-destructive">
            ...
            <RadioGroup value={selectedVariantId || ''} onValueChange={handleVariantChange}>
              <div className="space-y-2">
                {variants.map(variant => (
                  <label
                    key={variant.id}
                    className="flex items-center justify-between p-3 border rounded-lg cursor-pointer hover:border-primary transition-colors has-[:checked]:border-primary has-[:checked]:bg-primary/5"
                  >
...
export default EnhancedPriceSection;
